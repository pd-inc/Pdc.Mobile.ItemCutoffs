...
Keys: `connectionString`, `firebaseDatabaseUrl`, `firebaseServiceAccount`, `notifications:sqsQueueUrl` (the SAME queue `Pdc.Mobile` uses), `notifications:sqsRegion`, `ecommerce:cacheUri`, `ecommerce:cacheKey`, `cutoff:maxAttempts` (optional, default 3), `elasticsearch*`.
